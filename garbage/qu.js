...
function executeStep(stepId) {
  loadQuest(function (step) {
    ...
    if (!player.visitedSteps.includes(step.id))
      player.visitedSteps.push(step.id);
...
// ---------------
show() {
    const chatContainer = Doc("#chatContainer");
    const messageContainer = Doc("div").addClass("message");
    const authorElement = Doc("div").addClass("author").text(this.author);
    const avatarElement = this.avatar
      ? Doc("img")
          .addClass("avatar")
          .attr("src", `${location.pathname}img/persons/${this.avatar}`)
      : Doc("div").addClass("avatar").text(this.author[0]);
    const contentElement = Doc("div").addClass("content").text(this.content);
    ...
    messageContainer
      .addClass("left")
      .addClass(authorClasses[this.author] || "");

    chatContainer.append(avatarElement, contentElement);
